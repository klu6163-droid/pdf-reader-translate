...
> 以下三项已经完成工程评估，当前选择缓办是基于收益、改动面和维护成本的主动决策，
> **不是审计遗漏，也不表示现有实现存在待清零的阻断缺陷**。触发条件出现前不为拆分而拆分。

- **拆分 `backend/app/services/pdf_service.py`**
  - 当前决策：保留现有单模块。完整翻译、覆盖翻译、纯文本兜底共享较多流程，现阶段强拆会扩大回归面，用户侧收益有限。
  - 触发条件：下次实质修改翻译引擎时启动拆分，例如新增/替换翻译引擎、调整降级编排，或需要独立演进 pdf2zh、覆盖翻译、纯文本兜底中的任一条路径。

- **由 OpenAPI 自动生成前端类型和 API Client**
  - 当前决策：先采用已提交的 `backend/openapi.json` 快照门禁，继续维护轻量手写客户端；此时引入代码生成工具链会增加依赖、生成物和升级维护成本。
  - 触发条件：新增第二个 API 客户端（如 Web/移动端），或快照门禁上线后仍发生一次前后端字段漂移回归，再引入 OpenAPI → TypeScript 代码生成。

- **桌面 UI/Tauri 全链路自动化测试**
  - 当前决策：继续以 FastAPI HTTP 集成测试、前端组件测试、Rust 测试和 Windows 安装包健康冒烟分层覆盖；暂不引入依赖真实 WebView、文件对话框和进程生命周期的高维护 E2E 框架。
  - 触发条件：进入面向公众的持续版本发布阶段，或拖放、文件读写、后端崩溃重启等跨层流程再次出现人工验收未发现的回归时，再建设桌面端 E2E。
